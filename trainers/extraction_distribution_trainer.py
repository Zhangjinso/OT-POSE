...
class Trainer(BaseTrainer):
    ...
    def optimize_parameters(self, data):
        ...
        if source_skeleton.shape[2]==256:
            input_skeleton = torch.cat((torch.cat([source_skeleton,target_skeleton],1), torch.cat([target_skeleton,source_skeleton],1)), 0)
        ...
        self.gen_losses['attn_rec'] = self.criteria['attn_rec'](info, gt_image)
    ...
    def _start_of_iteration(self, data, current_iteration):
        ...
        # print(current_iteration, self.opt.trainer.gan_start_iteration)
    ...
    def _get_visualizations(self, data):
        ...
        with torch.no_grad():
            ...
            for i in range(len(attn_image)):
                attn_image[i] = F.interpolate(attn_image[i], source_image.shape[2:] )
    ...
    def test(self, data_loader, output_dir, current_iteration=-1):
        ...
        for it, data in enumerate(tqdm(data_loader)):
            ...
            if data['source_skeleton'].shape[2]==256:
            ...
            # output_images = torch.cat([input_image, data['target_image'], output_images],3)
